...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    public void setState(TransitionState newState) {
        if (newState != TransitionState.FINISHED || this.mCurrentState != -1) {
            ...
            switch (C15605.f369xabc7e4ac[oldState.ordinal()]) {
                case 1:
                case 2:
    ...
    static /* synthetic */ class C15605 {
        ...
        static {
            ...
            try {
                f369xabc7e4ac[TransitionState.SETUP.ordinal()] = 2;
            } catch (NoSuchFieldError e2) {
            }
            try {
                f369xabc7e4ac[TransitionState.MOVING.ordinal()] = 3;
            } catch (NoSuchFieldError e3) {
            }
            try {
                f369xabc7e4ac[TransitionState.FINISHED.ordinal()] = 4;
    ...
    private static class MyTracker implements MotionTracker {
        ...
        /* renamed from: me */
        private static MyTracker f370me = new MyTracker();
        ...
        public static MyTracker obtain() {
            f370me.tracker = VelocityTracker.obtain();
            return f370me;
        }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, C2299R.styleable.MotionLayout);
            int N = a.getIndexCount();
            boolean apply = true;
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C2299R.styleable.MotionLayout_layoutDescription) {
                    this.mScene = new MotionScene(getContext(), this, a.getResourceId(attr, -1));
                } else if (attr == C2299R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = a.getResourceId(attr, -1);
                } else if (attr == C2299R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = a.getFloat(attr, 0.0f);
                    this.mInTransition = true;
                } else if (attr == C2299R.styleable.MotionLayout_applyMotionScene) {
                    apply = a.getBoolean(attr, apply);
                } else {
                    int i2 = 0;
                    if (attr == C2299R.styleable.MotionLayout_showPaths) {
                    ...
                    } else if (attr == C2299R.styleable.MotionLayout_motionDebug) {
    ...
    public void updateStateAnimate(int stateId, ConstraintSet set, int duration) {
        if (this.mScene != null && this.mCurrentState == stateId) {
            updateState(C2299R.C2302id.view_transition, getConstraintSet(stateId));
            setState(C2299R.C2302id.view_transition, -1, -1);
            updateState(stateId, set);
            MotionScene.Transition tmpTransition = new MotionScene.Transition(-1, this.mScene, C2299R.C2302id.view_transition, stateId);
