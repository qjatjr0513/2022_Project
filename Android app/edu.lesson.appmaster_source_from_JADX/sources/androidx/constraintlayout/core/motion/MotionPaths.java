...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    public void initCartesian(MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = motionPaths.f335x;
        float position3 = position2;
        float position4 = motionPaths.f336y;
        float endCenterX = motionPaths2.f335x + (f / 2.0f);
        float endCenterY = motionPaths2.f336y + (f3 / 2.0f);
        float pathVectorX = endCenterX - (f5 + (f2 / 2.0f));
        float pathVectorY = endCenterY - (position4 + (f4 / 2.0f));
        this.f335x = (float) ((int) ((f5 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f336y = (float) ((int) ((position4 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        ...
        this.mMode = 0;
        this.f335x = (float) ((int) (((motionPaths.f335x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f336y = (float) ((int) (((motionPaths.f336y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f)));
    ...
    public void setupRelative(Motion mc, MotionPaths relative) {
        double dx = (double) (((this.f335x + (this.width / 2.0f)) - relative.f335x) - (relative.width / 2.0f));
        double dy = (double) (((this.f336y + (this.height / 2.0f)) - relative.f336y) - (relative.height / 2.0f));
        this.mRelativeToController = mc;
        this.f335x = (float) Math.hypot(dy, dx);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f336y = (float) (Math.atan2(dy, dx) + 1.5707963267948966d);
        } else {
            this.f336y = (float) Math.toRadians((double) this.mRelativeAngle);
        }
    }
    ...
    public void initScreen(int parentWidth, int parentHeight, MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = motionPaths.f335x;
        float f6 = position2;
        float position3 = motionPaths.f336y;
        float endCenterX = motionPaths2.f335x + (f / 2.0f);
        float endCenterY = motionPaths2.f336y + (f3 / 2.0f);
        this.f335x = (float) ((int) ((f5 + ((endCenterX - (f5 + (f2 / 2.0f))) * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f336y = (float) ((int) ((position3 + ((endCenterY - (position3 + (f4 / 2.0f))) * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        this.height = (float) ((int) (f4 + (scaleY * scaleHeight)));
        this.mMode = 2;
        if (!Float.isNaN(motionKeyPosition.mPercentX)) {
            this.f335x = (float) ((int) (motionKeyPosition.mPercentX * ((float) ((int) (((float) parentWidth) - this.width)))));
        }
        if (!Float.isNaN(motionKeyPosition.mPercentY)) {
            this.f336y = (float) ((int) (motionKeyPosition.mPercentY * ((float) ((int) (((float) parentHeight) - this.height)))));
        }
    ...
    public void initPath(MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f = motionPaths.f335x;
        float f2 = motionPaths.width;
        float f3 = motionPaths.f336y;
        float f4 = position2;
        float position3 = motionPaths.height;
        float endCenterX = motionPaths2.f335x + (motionPaths2.width / 2.0f);
        float endCenterY = motionPaths2.f336y + (motionPaths2.height / 2.0f);
        float pathVectorX = endCenterX - ((f2 / 2.0f) + f);
        float pathVectorY = endCenterY - (f3 + (position3 / 2.0f));
        this.f335x = (float) ((int) ((f + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f336y = (float) ((int) ((f3 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        ...
        float f6 = endCenterY;
        float f7 = (float) ((int) ((motionPaths3.f335x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f335x = f7;
        float f8 = pathVectorX;
        float f9 = (float) ((int) ((motionPaths3.f336y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.f336y = f9;
        this.f335x = f7 + ((-pathVectorY) * perpendicular);
        this.f336y = f9 + (pathVectorX * perpendicular);
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean diffx = diff(this.f335x, points.f335x);
        boolean diffy = diff(this.f336y, points.f336y);
        int c = 0 + 1;
    ...
    public void getCenter(double p, int[] toUse, double[] data, float[] point, int offset) {
        float f;
        int[] iArr = toUse;
        float v_x = this.f335x;
        float v_y = this.f336y;
    ...
    public void setView(float position2, MotionWidget view, int[] toUse, double[] data, double[] slope, double[] cycle) {
        ...
        int[] iArr = toUse;
        double[] dArr = slope;
        float v_x2 = this.f335x;
        float v_y2 = this.f336y;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f335x, this.f336y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
