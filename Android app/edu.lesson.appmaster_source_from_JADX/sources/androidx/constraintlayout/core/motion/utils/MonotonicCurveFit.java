...
public class MonotonicCurveFit extends CurveFit {
    ...
    /* renamed from: mT */
    private double[] f347mT;
    private double[][] mTangent;

    /* renamed from: mY */
    private double[][] f348mY;
    ...
    public void getPos(double t, double[] v) {
        double[] dArr = this.f347mT;
        int n = dArr.length;
        int dim = this.f348mY[0].length;
        if (this.mExtrapolate) {
            if (t <= dArr[0]) {
                getSlope(dArr[0], this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = this.f348mY[0][j] + ((t - this.f347mT[0]) * this.mSlopeTemp[j]);
                }
                return;
            } else if (t >= dArr[n - 1]) {
                getSlope(dArr[n - 1], this.mSlopeTemp);
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = this.f348mY[n - 1][j2] + ((t - this.f347mT[n - 1]) * this.mSlopeTemp[j2]);
                }
                return;
            }
        } else if (t <= dArr[0]) {
            for (int j3 = 0; j3 < dim; j3++) {
                v[j3] = this.f348mY[0][j3];
            }
            return;
        } else if (t >= dArr[n - 1]) {
            for (int j4 = 0; j4 < dim; j4++) {
                v[j4] = this.f348mY[n - 1][j4];
            }
            return;
        }
        for (int i = 0; i < n - 1; i++) {
            if (t == this.f347mT[i]) {
                for (int j5 = 0; j5 < dim; j5++) {
                    v[j5] = this.f348mY[i][j5];
                }
            }
            double[] dArr2 = this.f347mT;
            if (t < dArr2[i + 1]) {
                double h = dArr2[i + 1] - dArr2[i];
                double x = (t - dArr2[i]) / h;
                for (int j6 = 0; j6 < dim; j6++) {
                    double[][] dArr3 = this.f348mY;
    ...
    public void getPos(double t, float[] v) {
        ...
        if (this.mExtrapolate) {
            if (t <= dArr[0]) {
                getSlope(dArr[0], this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = (float) (this.f348mY[0][j] + ((t - this.f347mT[0]) * this.mSlopeTemp[j]));
                }
                return;
            } else if (t >= dArr[n - 1]) {
                getSlope(dArr[n - 1], this.mSlopeTemp);
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = (float) (this.f348mY[n - 1][j2] + ((t - this.f347mT[n - 1]) * this.mSlopeTemp[j2]));
                }
                return;
            }
        } else if (t <= dArr[0]) {
            for (int j3 = 0; j3 < dim; j3++) {
                v[j3] = (float) this.f348mY[0][j3];
            }
            return;
        } else if (t >= dArr[n - 1]) {
            for (int j4 = 0; j4 < dim; j4++) {
                v[j4] = (float) this.f348mY[n - 1][j4];
            }
            return;
        }
        for (int i = 0; i < n - 1; i++) {
    ...
    public double getPos(double t, int j) {
        ...
        if (this.mExtrapolate) {
            if (t <= dArr[0]) {
                return this.f348mY[0][i] + ((t - dArr[0]) * getSlope(dArr[0], i));
            }
            if (t >= dArr[n - 1]) {
                return this.f348mY[n - 1][i] + ((t - dArr[n - 1]) * getSlope(dArr[n - 1], i));
            }
        } else if (t <= dArr[0]) {
            return this.f348mY[0][i];
        } else {
            if (t >= dArr[n - 1]) {
                return this.f348mY[n - 1][i];
            }
        }
        for (int i2 = 0; i2 < n - 1; i2++) {
            double[] dArr2 = this.f347mT;
            if (t == dArr2[i2]) {
                return this.f348mY[i2][i];
            }
            if (t < dArr2[i2 + 1]) {
