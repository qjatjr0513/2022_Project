...
public class LinearCurveFit extends CurveFit {
    ...
    /* renamed from: mT */
    private double[] f345mT;
    private double mTotalLength = Double.NaN;

    /* renamed from: mY */
    private double[][] f346mY;
    ...
    private double getLength2D(double t) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            last_x = px;
            last_y = py;
            double[] dArr3 = this.f345mT;
            if (t == dArr3[i]) {
                return sum;
            }
            if (t < dArr3[i + 1]) {
                double x = (t - dArr3[i]) / (dArr3[i + 1] - dArr3[i]);
                double[][] dArr4 = this.f346mY;
                double x1 = dArr4[i][0];
    ...
    public void getPos(double t, double[] v) {
        double[] dArr = this.f345mT;
        int n = dArr.length;
        int dim = this.f346mY[0].length;
        if (this.mExtrapolate) {
            if (t <= dArr[0]) {
                getSlope(dArr[0], this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = this.f346mY[0][j] + ((t - this.f345mT[0]) * this.mSlopeTemp[j]);
                }
                return;
            } else if (t >= dArr[n - 1]) {
                getSlope(dArr[n - 1], this.mSlopeTemp);
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = this.f346mY[n - 1][j2] + ((t - this.f345mT[n - 1]) * this.mSlopeTemp[j2]);
                }
                return;
            }
        } else if (t <= dArr[0]) {
            for (int j3 = 0; j3 < dim; j3++) {
                v[j3] = this.f346mY[0][j3];
            }
            return;
        } else if (t >= dArr[n - 1]) {
            for (int j4 = 0; j4 < dim; j4++) {
                v[j4] = this.f346mY[n - 1][j4];
            }
            return;
        }
        for (int i = 0; i < n - 1; i++) {
            if (t == this.f345mT[i]) {
                for (int j5 = 0; j5 < dim; j5++) {
                    v[j5] = this.f346mY[i][j5];
                }
            }
            double[] dArr2 = this.f345mT;
            if (t < dArr2[i + 1]) {
                double x = (t - dArr2[i]) / (dArr2[i + 1] - dArr2[i]);
                for (int j6 = 0; j6 < dim; j6++) {
                    double[][] dArr3 = this.f346mY;
    ...
    public void getPos(double t, float[] v) {
        ...
        if (this.mExtrapolate) {
            if (t <= dArr[0]) {
                getSlope(dArr[0], this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = (float) (this.f346mY[0][j] + ((t - this.f345mT[0]) * this.mSlopeTemp[j]));
                }
                return;
            } else if (t >= dArr[n - 1]) {
                getSlope(dArr[n - 1], this.mSlopeTemp);
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = (float) (this.f346mY[n - 1][j2] + ((t - this.f345mT[n - 1]) * this.mSlopeTemp[j2]));
                }
                return;
            }
        } else if (t <= dArr[0]) {
            for (int j3 = 0; j3 < dim; j3++) {
                v[j3] = (float) this.f346mY[0][j3];
            }
            return;
        } else if (t >= dArr[n - 1]) {
            for (int j4 = 0; j4 < dim; j4++) {
                v[j4] = (float) this.f346mY[n - 1][j4];
            }
            return;
        }
        for (int i = 0; i < n - 1; i++) {
    ...
    public double getPos(double t, int j) {
        ...
        if (this.mExtrapolate) {
            if (t <= dArr[0]) {
                return this.f346mY[0][i] + ((t - dArr[0]) * getSlope(dArr[0], i));
            }
            if (t >= dArr[n - 1]) {
                return this.f346mY[n - 1][i] + ((t - dArr[n - 1]) * getSlope(dArr[n - 1], i));
            }
        } else if (t <= dArr[0]) {
            return this.f346mY[0][i];
        } else {
            if (t >= dArr[n - 1]) {
                return this.f346mY[n - 1][i];
            }
        }
        for (int i2 = 0; i2 < n - 1; i2++) {
            double[] dArr2 = this.f345mT;
            if (t == dArr2[i2]) {
                return this.f346mY[i2][i];
            }
            if (t < dArr2[i2 + 1]) {
