...
public class FlatBufferBuilder {
    ...
    public FlatBufferBuilder(int initial_size, ByteBufferFactory bb_factory2, ByteBuffer existing_bb, Utf8 utf82) {
        ...
        if (existing_bb != null) {
            this.f396bb = existing_bb;
            existing_bb.clear();
            this.f396bb.order(ByteOrder.LITTLE_ENDIAN);
        } else {
            this.f396bb = bb_factory2.newByteBuffer(initial_size);
        }
        this.utf8 = utf82;
        this.space = this.f396bb.capacity();
    }
    ...
    public int offset() {
        return this.f396bb.capacity() - this.space;
    }

    public void pad(int byte_size) {
        for (int i = 0; i < byte_size; i++) {
            ByteBuffer byteBuffer = this.f396bb;
            int i2 = this.space - 1;
            this.space = i2;
    ...
    public void prep(int size, int additional_bytes) {
        ...
        int align_size = ((~((this.f396bb.capacity() - this.space) + additional_bytes)) + 1) & (size - 1);
        while (this.space < align_size + size + additional_bytes) {
            int old_buf_size = this.f396bb.capacity();
            ByteBuffer old = this.f396bb;
            ByteBuffer growByteBuffer = growByteBuffer(old, this.bb_factory);
            this.f396bb = growByteBuffer;
            if (old != growByteBuffer) {
                this.bb_factory.releaseByteBuffer(old);
            }
            this.space += this.f396bb.capacity() - old_buf_size;
        }
        pad(align_size);
    }
    ...
    public ByteBuffer createUnintializedVector(int elem_size, int num_elems, int alignment) {
        ...
        ByteBuffer copy = this.f396bb.slice().order(ByteOrder.LITTLE_ENDIAN);
        copy.limit(length);
        return copy;
    }
    ...
    public <T extends Table> int createSortedVectorOfTables(T obj, int[] offsets) {
        obj.sortTables(offsets, this.f396bb);
    ...
    public int createString(CharSequence s) {
        ...
        this.space = i;
        byteBuffer.position(i);
        this.utf8.encodeUtf8(s, this.f396bb);
        return endVector();
    }
    ...
    public int createByteVector(byte[] arr, int offset, int length) {
        ...
        this.space = i;
        byteBuffer.position(i);
        this.f396bb.put(arr, offset, length);
        return endVector();
    }
    ...
    public int createByteVector(ByteBuffer byteBuffer) {
        ...
        startVector(1, length, 1);
        ByteBuffer byteBuffer2 = this.f396bb;
        int i = this.space - length;
        this.space = i;
        byteBuffer2.position(i);
        this.f396bb.put(byteBuffer);
        return endVector();
    }
    ...
    public int endTable() {
        ...
        while (true) {
            ...
            int vt1 = this.f396bb.capacity() - this.vtables[i2];
            int vt2 = this.space;
            short len = this.f396bb.getShort(vt1);
            if (len == this.f396bb.getShort(vt2)) {
                int j = 2;
                while (j < len) {
                    if (this.f396bb.getShort(vt1 + j) == this.f396bb.getShort(vt2 + j)) {
                        j += 2;
                    }
                }
        ...
        if (existing_vtable != 0) {
            int capacity = this.f396bb.capacity() - vtableloc;
            this.space = capacity;
            this.f396bb.putInt(capacity, existing_vtable - vtableloc);
        } else {
    ...
    public void required(int table, int field) {
        int table_start = this.f396bb.capacity() - table;
        if (!(this.f396bb.getShort((table_start - this.f396bb.getInt(table_start)) + field) != 0)) {
    ...
    public void finish(int root_table, boolean size_prefix) {
        ...
        if (size_prefix) {
            addInt(this.f396bb.capacity() - this.space);
        }
        this.f396bb.position(this.space);
        this.finished = true;
    }
    ...
    public byte[] sizedByteArray(int start, int length) {
        ...
        this.f396bb.position(start);
        this.f396bb.get(array);
        return array;
    }

    public byte[] sizedByteArray() {
        return sizedByteArray(this.space, this.f396bb.capacity() - this.space);
    }

    public InputStream sizedInputStream() {
        finished();
        ByteBuffer duplicate = this.f396bb.duplicate();
        duplicate.position(this.space);
        duplicate.limit(this.f396bb.capacity());
