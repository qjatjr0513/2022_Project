...
public class FlexBuffersBuilder {
    ...
    /* renamed from: bb */
    public final ReadWriteBuf f399bb;
    ...
    public FlexBuffersBuilder(ReadWriteBuf bb, int flags2) {
        ...
        this.keyComparator = new Comparator<Value>() {
            public int compare(Value o1, Value o2) {
                ...
                int io = o2.key;
                do {
                    c1 = FlexBuffersBuilder.this.f399bb.get(ia);
                    c2 = FlexBuffersBuilder.this.f399bb.get(io);
                    if (c1 == 0) {
                        return c1 - c2;
                    }
                    ia++;
    ...
    private int putKey(String key) {
        ...
        int pos = this.f399bb.writePosition();
        if ((this.flags & 1) != 0) {
            ...
            this.f399bb.put(keyBytes, 0, keyBytes.length);
            this.f399bb.put((byte) 0);
            this.keyPool.put(key, Integer.valueOf(pos));
            return pos;
        }
        byte[] keyBytes2 = key.getBytes(StandardCharsets.UTF_8);
        this.f399bb.put(keyBytes2, 0, keyBytes2.length);
        this.f399bb.put((byte) 0);
        this.keyPool.put(key, Integer.valueOf(pos));
        return pos;
    }
    ...
    private Value writeBlob(int key, byte[] blob, int type, boolean trailing) {
        ...
        int sloc = this.f399bb.writePosition();
        this.f399bb.put(blob, 0, blob.length);
        if (trailing) {
            this.f399bb.put((byte) 0);
        }
    ...
    private int align(int alignment) {
        ...
        int padBytes = Value.paddingBytes(this.f399bb.writePosition(), byteWidth);
        while (true) {
    ...
    private void writeInt(long value, int byteWidth) {
        switch (byteWidth) {
            case 1:
                this.f399bb.put((byte) ((int) value));
                return;
            case 2:
                this.f399bb.putShort((short) ((int) value));
                return;
            case 4:
                this.f399bb.putInt((int) value);
                return;
            case 8:
                this.f399bb.putLong(value);
                return;
            default:
                return;
        }
    ...
    public ByteBuffer finish() {
        if (this.stack.size() == 1) {
            int byteWidth = align(this.stack.get(0).elemWidth(this.f399bb.writePosition(), 0));
            writeAny(this.stack.get(0), byteWidth);
            this.f399bb.put(this.stack.get(0).storedPackedType());
            this.f399bb.put((byte) byteWidth);
            this.finished = true;
            return ByteBuffer.wrap(this.f399bb.data(), 0, this.f399bb.writePosition());
        }
        throw new AssertionError();
    }
    ...
    private Value createVector(int key, int start, int length, boolean typed, boolean fixed, Value keys) {
        ...
        if (!fixed || typed) {
            ...
            int prefixElems = 1;
            if (value != null) {
                bitWidth = Math.max(bitWidth, value.elemWidth(this.f399bb.writePosition(), 0));
                prefixElems = 1 + 2;
            }
            int vectorType = 4;
            for (int i5 = start; i5 < this.stack.size(); i5++) {
                bitWidth = Math.max(bitWidth, this.stack.get(i5).elemWidth(this.f399bb.writePosition(), i5 + prefixElems));
                if (!typed) {
                    int i6 = start;
                } else if (i5 == start) {
    ...
    private void writeOffset(long val, int byteWidth) {
        int reloff = (int) (((long) this.f399bb.writePosition()) - val);
    ...
    private void writeDouble(double val, int byteWidth) {
        if (byteWidth == 4) {
            this.f399bb.putFloat((float) val);
        } else if (byteWidth == 8) {
            this.f399bb.putDouble(val);
        }
    }
    ...
    private Value createKeyVector(int start, int length) {
        ...
        for (int i = start; i < this.stack.size(); i++) {
            bitWidth = Math.max(bitWidth, Value.elemWidth(4, 0, (long) this.stack.get(i).key, this.f399bb.writePosition(), i + 1));
        }
