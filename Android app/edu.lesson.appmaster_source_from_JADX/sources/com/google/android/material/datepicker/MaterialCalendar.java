...
public final class MaterialCalendar<S> extends PickerFragment<S> {
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ...
        if (MaterialDatePicker.isFullscreen(themedContext)) {
            layout = C2436R.layout.mtrl_calendar_vertical;
            orientation = 1;
        } else {
            layout = C2436R.layout.mtrl_calendar_horizontal;
            orientation = 0;
        }
        View root = themedInflater.inflate(layout, viewGroup, false);
        root.setMinimumHeight(getDialogPickerHeight(requireContext()));
        GridView daysHeader = (GridView) root.findViewById(C2436R.C2439id.mtrl_calendar_days_of_week);
        ...
        this.recyclerView = (RecyclerView) root.findViewById(C2436R.C2439id.mtrl_calendar_months);
        ...
        int columns = themedContext.getResources().getInteger(C2436R.integer.mtrl_calendar_year_selector_span);
        RecyclerView recyclerView2 = (RecyclerView) root.findViewById(C2436R.C2439id.mtrl_calendar_year_selector_frame);
        ...
        if (root.findViewById(C2436R.C2439id.month_navigation_fragment_toggle) != null) {
    ...
    static int getDayHeight(Context context) {
        return context.getResources().getDimensionPixelSize(C2436R.dimen.mtrl_calendar_day_height);
    }
    ...
    private void addActionsToMonthNavigation(View root, final MonthsPagerAdapter monthsPagerAdapter) {
        final MaterialButton monthDropSelect = (MaterialButton) root.findViewById(C2436R.C2439id.month_navigation_fragment_toggle);
        ...
        ViewCompat.setAccessibilityDelegate(monthDropSelect, new AccessibilityDelegateCompat() {
            public void onInitializeAccessibilityNodeInfo(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
                ...
                if (MaterialCalendar.this.dayFrame.getVisibility() == 0) {
                    str = MaterialCalendar.this.getString(C2436R.string.mtrl_picker_toggle_to_year_selection);
                } else {
                    str = MaterialCalendar.this.getString(C2436R.string.mtrl_picker_toggle_to_day_selection);
                }
                accessibilityNodeInfoCompat.setHintText(str);
            }
        });
        MaterialButton monthPrev = (MaterialButton) root.findViewById(C2436R.C2439id.month_navigation_previous);
        monthPrev.setTag(NAVIGATION_PREV_TAG);
        MaterialButton monthNext = (MaterialButton) root.findViewById(C2436R.C2439id.month_navigation_next);
        monthNext.setTag(NAVIGATION_NEXT_TAG);
        this.yearFrame = root.findViewById(C2436R.C2439id.mtrl_calendar_year_selector_frame);
        this.dayFrame = root.findViewById(C2436R.C2439id.mtrl_calendar_day_selector_frame);
        setSelector(CalendarSelector.DAY);
    ...
    private static int getDialogPickerHeight(Context context) {
        ...
        int navigationHeight = resources.getDimensionPixelSize(C2436R.dimen.mtrl_calendar_navigation_height) + resources.getDimensionPixelOffset(C2436R.dimen.mtrl_calendar_navigation_top_padding) + resources.getDimensionPixelOffset(C2436R.dimen.mtrl_calendar_navigation_bottom_padding);
        int daysOfWeekHeight = resources.getDimensionPixelSize(C2436R.dimen.mtrl_calendar_days_of_week_height);
        return navigationHeight + daysOfWeekHeight + (MonthAdapter.MAXIMUM_WEEKS * resources.getDimensionPixelSize(C2436R.dimen.mtrl_calendar_day_height)) + ((MonthAdapter.MAXIMUM_WEEKS - 1) * resources.getDimensionPixelOffset(C2436R.dimen.mtrl_calendar_month_vertical_padding)) + resources.getDimensionPixelOffset(C2436R.dimen.mtrl_calendar_bottom_padding);
    }
