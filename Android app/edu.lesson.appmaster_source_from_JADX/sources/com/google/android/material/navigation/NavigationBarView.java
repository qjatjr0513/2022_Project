...
public abstract class NavigationBarView extends FrameLayout {
    ...
    public NavigationBarView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TintTypedArray attributes = ThemeEnforcement.obtainTintedStyledAttributes(context2, attrs, C2436R.styleable.NavigationBarView, defStyleAttr, defStyleRes, C2436R.styleable.NavigationBarView_itemTextAppearanceInactive, C2436R.styleable.NavigationBarView_itemTextAppearanceActive);
        ...
        if (attributes.hasValue(C2436R.styleable.NavigationBarView_itemIconTint)) {
            createNavigationBarMenuView.setIconTintList(attributes.getColorStateList(C2436R.styleable.NavigationBarView_itemIconTint));
        } else {
            createNavigationBarMenuView.setIconTintList(createNavigationBarMenuView.createDefaultColorStateList(16842808));
        }
        setItemIconSize(attributes.getDimensionPixelSize(C2436R.styleable.NavigationBarView_itemIconSize, getResources().getDimensionPixelSize(C2436R.dimen.mtrl_navigation_bar_item_default_icon_size)));
        if (attributes.hasValue(C2436R.styleable.NavigationBarView_itemTextAppearanceInactive)) {
            setItemTextAppearanceInactive(attributes.getResourceId(C2436R.styleable.NavigationBarView_itemTextAppearanceInactive, 0));
        }
        if (attributes.hasValue(C2436R.styleable.NavigationBarView_itemTextAppearanceActive)) {
            setItemTextAppearanceActive(attributes.getResourceId(C2436R.styleable.NavigationBarView_itemTextAppearanceActive, 0));
        }
        if (attributes.hasValue(C2436R.styleable.NavigationBarView_itemTextColor)) {
            setItemTextColor(attributes.getColorStateList(C2436R.styleable.NavigationBarView_itemTextColor));
        }
        if (getBackground() == null || (getBackground() instanceof ColorDrawable)) {
            ViewCompat.setBackground(this, createMaterialShapeDrawableBackground(context2));
        }
        if (attributes.hasValue(C2436R.styleable.NavigationBarView_itemPaddingTop)) {
            setItemPaddingTop(attributes.getDimensionPixelSize(C2436R.styleable.NavigationBarView_itemPaddingTop, 0));
        }
        if (attributes.hasValue(C2436R.styleable.NavigationBarView_itemPaddingBottom)) {
            setItemPaddingBottom(attributes.getDimensionPixelSize(C2436R.styleable.NavigationBarView_itemPaddingBottom, 0));
        }
        if (attributes.hasValue(C2436R.styleable.NavigationBarView_elevation)) {
            setElevation((float) attributes.getDimensionPixelSize(C2436R.styleable.NavigationBarView_elevation, 0));
        }
        ColorStateList backgroundTint = MaterialResources.getColorStateList(context2, attributes, C2436R.styleable.NavigationBarView_backgroundTint);
        DrawableCompat.setTintList(getBackground().mutate(), backgroundTint);
        setLabelVisibilityMode(attributes.getInteger(C2436R.styleable.NavigationBarView_labelVisibilityMode, -1));
        int itemBackground = attributes.getResourceId(C2436R.styleable.NavigationBarView_itemBackground, 0);
        if (itemBackground != 0) {
            createNavigationBarMenuView.setItemBackgroundRes(itemBackground);
        } else {
            setItemRippleColor(MaterialResources.getColorStateList(context2, attributes, C2436R.styleable.NavigationBarView_itemRippleColor));
        }
        int activeIndicatorStyleResId = attributes.getResourceId(C2436R.styleable.NavigationBarView_itemActiveIndicatorStyle, 0);
        if (activeIndicatorStyleResId != 0) {
            setItemActiveIndicatorEnabled(true);
            TypedArray activeIndicatorAttributes = context2.obtainStyledAttributes(activeIndicatorStyleResId, C2436R.styleable.NavigationBarActiveIndicator);
            setItemActiveIndicatorWidth(activeIndicatorAttributes.getDimensionPixelSize(C2436R.styleable.NavigationBarActiveIndicator_android_width, 0));
            setItemActiveIndicatorHeight(activeIndicatorAttributes.getDimensionPixelSize(C2436R.styleable.NavigationBarActiveIndicator_android_height, 0));
            setItemActiveIndicatorMarginHorizontal(activeIndicatorAttributes.getDimensionPixelOffset(C2436R.styleable.NavigationBarActiveIndicator_marginHorizontal, 0));
            setItemActiveIndicatorColor(MaterialResources.getColorStateList(context2, activeIndicatorAttributes, C2436R.styleable.NavigationBarActiveIndicator_android_color));
            ColorStateList colorStateList = backgroundTint;
            setItemActiveIndicatorShapeAppearance(ShapeAppearanceModel.builder(context2, activeIndicatorAttributes.getResourceId(C2436R.styleable.NavigationBarActiveIndicator_shapeAppearance, 0), 0).build());
            activeIndicatorAttributes.recycle();
        }
        if (attributes.hasValue(C2436R.styleable.NavigationBarView_menu)) {
            inflateMenu(attributes.getResourceId(C2436R.styleable.NavigationBarView_menu, 0));
        }
        attributes.recycle();
