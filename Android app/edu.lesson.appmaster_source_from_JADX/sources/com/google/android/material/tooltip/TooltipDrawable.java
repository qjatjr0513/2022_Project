...
public class TooltipDrawable extends MaterialShapeDrawable implements TextDrawableHelper.TextDrawableDelegate {
    private static final int DEFAULT_STYLE = C2436R.style.Widget_MaterialComponents_Tooltip;
    private static final int DEFAULT_THEME_ATTR = C2436R.attr.tooltipStyle;
    ...
    private void loadFromAttributes(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TypedArray a = ThemeEnforcement.obtainStyledAttributes(this.context, attrs, C2436R.styleable.Tooltip, defStyleAttr, defStyleRes, new int[0]);
        this.arrowSize = this.context.getResources().getDimensionPixelSize(C2436R.dimen.mtrl_tooltip_arrowSize);
        setShapeAppearanceModel(getShapeAppearanceModel().toBuilder().setBottomEdge(createMarkerEdge()).build());
        setText(a.getText(C2436R.styleable.Tooltip_android_text));
        TextAppearance textAppearance = MaterialResources.getTextAppearance(this.context, a, C2436R.styleable.Tooltip_android_textAppearance);
        if (textAppearance != null && a.hasValue(C2436R.styleable.Tooltip_android_textColor)) {
            textAppearance.setTextColor(MaterialResources.getColorStateList(this.context, a, C2436R.styleable.Tooltip_android_textColor));
        }
        setTextAppearance(textAppearance);
        setFillColor(ColorStateList.valueOf(a.getColor(C2436R.styleable.Tooltip_backgroundTint, MaterialColors.layer(ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, 16842801, cls.getCanonicalName()), 229), ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, C2436R.attr.colorOnBackground, cls.getCanonicalName()), 153)))));
        setStrokeColor(ColorStateList.valueOf(MaterialColors.getColor(this.context, C2436R.attr.colorSurface, cls.getCanonicalName())));
        this.padding = a.getDimensionPixelSize(C2436R.styleable.Tooltip_android_padding, 0);
        this.minWidth = a.getDimensionPixelSize(C2436R.styleable.Tooltip_android_minWidth, 0);
        this.minHeight = a.getDimensionPixelSize(C2436R.styleable.Tooltip_android_minHeight, 0);
        this.layoutMargin = a.getDimensionPixelSize(C2436R.styleable.Tooltip_android_layout_margin, 0);
        a.recycle();
    }
